...
use jsonrpsee_server::{
    middleware::rpc::RpcServiceBuilder, stop_channel, ws, ConnectionGuard, ConnectionState,
    HttpBody, HttpRequest, Methods, Server, ServerConfig, WsExtensionFactory,
};
use soketto::{
    base::Header,
...
async fn low_level_connect_instantiates_extensions_per_connection() {
    ...
    let (response, _connection) = ws::connect(
        upgrade_request(),
        server_config,
        Methods::new(),
        connection,
