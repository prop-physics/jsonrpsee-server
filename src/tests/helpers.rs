...
use crate::{
    serve_with_graceful_shutdown, stop_channel, RpcModule, Server, ServerBuilder, ServerHandle,
};
...
pub(crate) async fn server_with_handles() -> (SocketAddr, ServerHandle) {
    ...
    module
        ...
        .unwrap();
    module
        .register_async_method::<Result<u64, ErrorObjectOwned>, _, _>(
            "add_async",
        ...
        .unwrap();
    module
        .register_method("invalid_params", |_params, _, _| {
            Err::<(), _>(invalid_params())
        })
        .unwrap();
    module
        .register_method("call_fail", |_params, _, _| Err::<(), _>(MyAppError))
        .unwrap();
    module
